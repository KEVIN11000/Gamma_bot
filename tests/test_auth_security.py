...
class TestAuthSecurity(unittest.TestCase):
    ...
    def _make_app(self, env_overrides: dict):
        ...
        with patch('com.bot.GAMMA') as MockGamma, \
             patch('logic.cron_jobs.notificacion_clima'), \
             patch('logic.cron_jobs.resumen_semanal'), \
             patch('logic.cron_jobs.rotar_logs'):
    ...
    def test_cron_200_with_header_secret(self):
        ...
        with patch('flask_app.notificacion_clima', return_value=True):
            resp = client.get('/cron/clima',
                              headers={'X-Cron-Secret': 'my_secret'})
