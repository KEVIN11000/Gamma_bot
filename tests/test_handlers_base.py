...
def test_comando_debug_dev_app_not_exists(
    mock_exists, mock_safe, mock_auth, bot_mock, mock_message
):
    mock_auth.return_value = lambda f: f
    mock_safe.return_value = lambda f: f
...
@patch("com.handlers.base.safe_handler")
@patch("com.handlers.base.Path.exists")
def test_comando_debug_dev_app_exists_empty(
    mock_exists, mock_safe, mock_auth, bot_mock, mock_message
):
    mock_auth.return_value = lambda f: f
    mock_safe.return_value = lambda f: f
    ...
    with patch.dict(os.environ, {"MODO_DESARROLLADOR": "True"}), patch(
        "builtins.open", m_open
    ):
        handler(mock_message)
        bot_mock.reply_to.assert_called_once()
        assert (
            "[El archivo existe pero está vacío]" in bot_mock.reply_to.call_args[0][1]
        )
...
@patch("com.handlers.base.safe_handler")
@patch("com.handlers.base.Path.exists")
def test_comando_debug_dev_app_exists_content(
    mock_exists, mock_safe, mock_auth, bot_mock, mock_message
):
    mock_auth.return_value = lambda f: f
    mock_safe.return_value = lambda f: f
...
@patch("com.handlers.base.safe_handler")
@patch("com.handlers.base.Path.exists")
def test_comando_comandos_prod(
    mock_exists, mock_safe, mock_auth, bot_mock, mock_message
):
    mock_auth.return_value = lambda f: f
    mock_safe.return_value = lambda f: f
    ...
    with patch.dict(os.environ, {"MODO_DESARROLLADOR": "False"}), patch(
        "builtins.open", m_open
    ):
        handler(mock_message)
        bot_mock.reply_to.assert_called_once()
