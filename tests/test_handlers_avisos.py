...
def test_comando_aviso_with_args(mock_bot, mock_gamma_app, mock_message):
    ...
    with patch("com.handlers.avisos._procesar_frase_aviso") as mock_procesar:
        ...
        mock_procesar.assert_called_once_with(
            mock_message, "entregar laboratorio mañana", mock_bot, mock_gamma_app
        )
...
def test_callback_aviso_cancelar(mock_estado, mock_bot, mock_gamma_app, mock_call):
    ...
    aviso_handler = next(
        f for func, f in handlers if func(MagicMock(data="aviso_test"))
    )
...
@patch("com.handlers.avisos.EstadoGestor")
def test_callback_aviso_confirmar_expired(
    mock_estado, mock_bot, mock_gamma_app, mock_call
):
    handlers = get_callback_handlers(mock_bot, mock_gamma_app)
    aviso_handler = next(
        f for func, f in handlers if func(MagicMock(data="aviso_test"))
...
@patch("com.handlers.avisos.EstadoGestor")
def test_callback_aviso_confirmar_success(
    mock_estado, mock_enqueue, mock_bot, mock_gamma_app, mock_call
):
    handlers = get_callback_handlers(mock_bot, mock_gamma_app)
    aviso_handler = next(
        f for func, f in handlers if func(MagicMock(data="aviso_test"))
    ...
    mock_enqueue.assert_called_once_with(
        12345, "crear_aviso_calendar", {"titulo": "Test", "fecha": "2023-10-10"}
    )
    mock_estado.pop.assert_called_once_with(12345)
...
def test_comando_avisos_telegram_exception(mock_bot, mock_gamma_app, mock_message):
    ...
    mock_gamma_app.agente_excel.obtener_lista_avisos_calendar.side_effect = (
        telebot.apihelper.ApiTelegramException(
            "metod", "result", {"error_code": 400, "description": "Bad Request"}
        )
    )
...
def test_comando_avisos_general_exception(mock_bot, mock_gamma_app, mock_message):
    ...
    mock_gamma_app.agente_excel.obtener_lista_avisos_calendar.side_effect = Exception(
        "General Error"
    )
...
def test_callback_borrar_aviso_success_and_empty(mock_bot, mock_gamma_app, mock_call):
    handlers = get_callback_handlers(mock_bot, mock_gamma_app)
    borrar_handler = next(
        f for func, f in handlers if func(MagicMock(data="borrar_test"))
    )
...
def test_capturar_frase_aviso_secuencial_invalid_not_verified(
...
@patch("com.handlers.avisos.verificar_usuario_manual")
def test_capturar_frase_aviso_secuencial_invalid_text(
    mock_verificar, mock_bot, mock_gamma_app, mock_message
):
...
def test_capturar_frase_aviso_secuencial_valid(
    mock_procesar, mock_verificar, mock_bot, mock_gamma_app, mock_message
):
    mock_verificar.return_value = True
    ...
    mock_procesar.assert_called_once_with(
        mock_message, "entregar tp", mock_bot, mock_gamma_app
    )


@patch("com.handlers.avisos.AIService")
@patch("com.handlers.avisos.EstadoGestor")
def test_procesar_frase_aviso_success(
    mock_estado, mock_aiservice, mock_bot, mock_gamma_app, mock_message
):
    mock_msg_espera = MagicMock()
    mock_msg_espera.message_id = 999
    mock_bot.send_message.return_value = mock_msg_espera

    datos_ia = {"titulo": "Entregar tp", "fecha": "Mañana", "hora": "10:00"}
    ...
    mock_aiservice.interpretar_frase_con_ia.assert_called_once_with(
        "entregar tp mañana"
    )
...
@patch("com.handlers.avisos.AIService")
def test_procesar_frase_aviso_error(
    mock_aiservice, mock_bot, mock_gamma_app, mock_message
):
    mock_msg_espera = MagicMock()
    mock_msg_espera.message_id = 999
    mock_bot.send_message.return_value = mock_msg_espera
