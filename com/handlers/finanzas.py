...
def register_finanzas_handlers(bot: TeleBot, gamma_app):
    ...
    def comando_movimiento_financiero(message):
        ...
        if len(partes) < 2:
            bot.reply_to(message, f"❌ Falta descripción. Ejemplo:\n`/{comando} 50000 Hamburguesa`", parse_mode="Markdown")
            return
    ...
    def manejar_foto_ticket(message):
        msg_carga = bot.reply_to(message, "⏳ Descargando y analizando imagen con OCR (Gemini 2.5)...")
        ...
        try:
            ...
            bot.edit_message_text(texto_confirmacion, message.chat.id, msg_carga.message_id, parse_mode="Markdown", reply_markup=teclado)
            
        except Exception as e:
            logger.error(f"❌ Error procesando foto OCR: {e}")
            bot.edit_message_text(f"❌ Hubo un problema al procesar la imagen: {e}", message.chat.id, msg_carga.message_id)
    ...
    def callback_ocr(call):
        try:
            ...
            if accion == "no":
                ...
                bot.edit_message_text("❌ Gasto descartado.", call.message.chat.id, call.message.message_id)
                return
            ...
            if accion == "ok":
                ...
                resultado = gamma_app.agente_financiero.registrar_movimiento(datos)
                bot.edit_message_text(resultado, call.message.chat.id, call.message.message_id)
